...
class DateTools:
    ...
    __slots__ = ()
    _hx_statics = ["DAYS_OF_MONTH", "getMonthDays"]
    ...
    def getMonthDays(d):
        ...
        year = d.date.year
        if (month != 1):
            return python_internal_ArrayImpl._get(DateTools.DAYS_OF_MONTH, month)
        isB = (((HxOverrides.mod(year, 4) == 0) and ((HxOverrides.mod(year, 100) != 0))) or ((HxOverrides.mod(year, 400) == 0)))
        if isB:
            return 29
        else:
            return 28
...
class Main:
    ...
    def main():
        now = Date.now()
        year = now.date.year
        month = (now.date.month - 1)
        monthNames = ["1月", "2月", "3月", "4月", "5月", "6月", "7月", "8月", "9月", "10月", "11月", "12月"]
        _hx_str = Std.string((((("--- " + Std.string(year)) + "年 ") + HxOverrides.stringOrNull((monthNames[month] if month >= 0 and month < len(monthNames) else None))) + " ---"))
        python_Lib.printString((("" + ("null" if _hx_str is None else _hx_str)) + HxOverrides.stringOrNull(python_Lib.lineEnd)))
        _hx_str = "日 月 火 水 木 金 土"
        python_Lib.printString((("" + ("null" if _hx_str is None else _hx_str)) + HxOverrides.stringOrNull(python_Lib.lineEnd)))
        firstDay = HxOverrides.mod(Date(year,month,1,0,0,0).date.isoweekday(), 7)
        daysInMonth = DateTools.getMonthDays(now)
        line = ""
        _g = 0
        _g1 = firstDay
        while (_g < _g1):
        ...
        while (_g < _g1):
            day = _g
            _g = (_g + 1)
            line = (("null" if line is None else line) + HxOverrides.stringOrNull(((HxOverrides.stringOrNull(StringTools.lpad(Std.string(day)," ",2)) + " "))))
            if ((HxOverrides.mod(((day + firstDay)), 7) == 0) or ((day == daysInMonth))):
